...
cd && \
mkdir -p /root/rlsv2_bak && \
cp /root/rl-swarm/swarm.pem /root/rlsv2_bak/swarm.pem && \
cp /root/rl-swarm/userApiKey.json /root/rlsv2_bak/userApiKey.json && \
cp /root/rl-swarm/userData.json /root/rlsv2_bak/userData.json && \
rm -rf rl-swarm && \
git clone https://github.com/gensyn-ai/rl-swarm && \
cd rl-swarm && python3 -m venv .venv && \
cp /root/rlsv2_bak/swarm.pem /root/rl-swarm/swarm.pem && \
cp /root/rlsv2_bak/userApiKey.json /root/rl-swarm/userApiKey.json && \
cp /root/rlsv2_bak/userData.json /root/rl-swarm/userData.json && \
