...
if [ "$CONNECT_TO_TESTNET" = true ]; then
    ...
    if [ -f "$ROOT/userData.json" ]; then
        ...
        wget https://github.com/cloudflare/cloudflared/releases/download/2025.6.1/cloudflared-linux-arm64
        chmod +x cloudflared-linux-arm64
        nohup ./cloudflared-linux-arm64 tunnel --url http://localhost:3000 > "$ROOT/logs/cloudflared.log" 2>&1 &
        sleep 10 
